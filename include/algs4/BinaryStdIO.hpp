...
namespace BinaryStdIO {
    namespace detail {
        ...
        inline void writeByte(std::byte byte) {
        ...
        inline bool isEmpty() { return inN_ < 0; }
        ...
        inline std::byte readByte() {
            assert(!isEmpty());
            if (inN_ == 0) fillInBuffer();
            auto x = inBuffer_;
            if (inN_ == std::numeric_limits<unsigned char>::digits)
                fillInBuffer();  // special case when aligned byte
            else {
    ...
    inline void write(const std::string& x) {
        for (char c : x) write(c);
    }

    using detail::isEmpty;

    template <typename T>
        requires std::is_same_v<std::remove_cvref_t<T>, bool>
    T read() {
        return detail::readBit();
    }

    template <std::integral T>
        requires(!std::is_same_v<std::remove_cvref_t<T>, bool> && sizeof(T) == 1)
    T read() {
        return std::to_integer<T>(detail::readByte());
    }
    ...
    T read(int r) {
        using namespace detail;
        if (r == std::numeric_limits<T>::digits + (std::numeric_limits<T>::is_signed ? 1 : 0)) return read<T>();
        assert(r >= 1 && r <= std::numeric_limits<T>::digits + (std::numeric_limits<T>::is_signed ? 1 : 0));
        T x = 0;
        for (int i = 0; i < r; ++i) {
            x <<= 1;
