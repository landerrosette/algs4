...
namespace detail {
    ...
    public:
        ...
        friend constexpr decltype(auto) iter_move(const MonodirectionalIterator& i) noexcept(
            noexcept(std::ranges::iter_move(i.base()))) {
            return std::ranges::iter_move(i.current_);
        }

        template <std::forward_iterator I>
            requires std::indirectly_swappable<I, Iter>
        friend constexpr void iter_swap(const MonodirectionalIterator& l, const MonodirectionalIterator<I>& r) noexcept(
            noexcept(std::ranges::iter_swap(l.current_, r.current_))) {
            std::ranges::iter_swap(l.current_, r.current_);
        }
    };
}  // namespace detail
}  // namespace algs4
