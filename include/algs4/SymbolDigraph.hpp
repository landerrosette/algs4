...
public:
    SymbolDigraph(const std::filesystem::path& filepath, char sp) {
        ...
        if (!in.is_open()) throw std::invalid_argument("Cannot open file: " + filepath.string());
        ...
        for (std::string line; std::getline(in, line);) {
            ...
            for (std::istringstream iss(line); std::getline(iss, name, sp);)
                if (!st_.contains(name)) st_.put(name, static_cast<int>(st_.size()));
        }
        keys_.resize(st_.size());  // Inverted index to get string keys is an array.
        for (const auto& name : st_.keys()) keys_[*st_.get(name)] = name;
        ...
        for (std::string line; std::getline(in, line);) {
            ...
            int v = *st_.get(name);
            while (std::getline(iss, name, sp)) G_->addEdge(v, *st_.get(name));
        }
    }
