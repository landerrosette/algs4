...
constexpr void algs4::detail::CCBase::dfs(const GraphBase<int> &G, int v) {
    marked_[v] = true;
    id_[v] = count_;
    for (int w: G.adj(v))
...
constexpr bool algs4::detail::CCBase::connected(int v, int w) const {
    assert(v >= 0 && v < std::ssize(marked_));
    assert(w >= 0 && w < std::ssize(marked_));
    return id_[v] == id_[w];
}

constexpr int algs4::detail::CCBase::id(int v) const {
    assert(v >= 0 && v < std::ssize(marked_));
    return id_[v];
}
