...
SymbolDigraph::SymbolDigraph(const std::filesystem::path &dataFilePath, char sp) {
    {
        ...
        std::string line;
        while (getline(dataFile, line)) {            // 构造索引
            std::string name;
            std::istringstream iss(line);
            while (getline(iss, name, sp))      // 为每个不同的字符串关联一个索引
                if (!st.contains(name)) st.put(name, st.size());
        }
    }
    keys = std::vector<std::string>(st.size());         // 用来获得顶点名的反向索引是一个数组
    for (const auto &name: st.getKeys()) {
    ...
    G = std::make_unique<Digraph>(st.size());
    {
        ...
        std::string line;
        while (getline(dataFile, line)) {            // 构造图
            std::string name;
            std::istringstream iss(line);
            getline(iss, name, sp);
            int v = *st.get(name);
            while (getline(iss, name, sp))      // 将每一行的第一个顶点和该行的其他顶点相连
