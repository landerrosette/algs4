...
void algs4::SP::relax(const EdgeWeightedDigraph &G, int v) {
...
algs4::SP::SP(const EdgeWeightedDigraph &G, int s)
    : edgeTo(G.V()),
      distTo_(G.V(), std::numeric_limits<double>::infinity()) {
    distTo_[s] = 0.0;
}

std::list<algs4::DirectedEdge> algs4::SP::pathTo(int v) const {
